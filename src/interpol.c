...
/* exchange gridpoint NY+1 */
  ...
  MPI_Bsend(&int_top_to_bot[1],NX,MPI_FLOAT,INDEX[3],3,SHOT_COMM);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&int_top_to_bot[1],NX,MPI_FLOAT,INDEX[4],3,SHOT_COMM,&status);
...
/* exchange gridpoint NX+1 */
  ...
  MPI_Bsend(&int_lef_to_rig[1],NY,MPI_FLOAT,INDEX[1],3,SHOT_COMM);
  MPI_Barrier(SHOT_COMM);
  MPI_Recv(&int_lef_to_rig[1],NY,MPI_FLOAT,INDEX[2],3,SHOT_COMM,&status);
