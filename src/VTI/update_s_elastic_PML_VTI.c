/*  Update_s_elastic_PML_VTI
 *
 *  updating stress components at gridpoints [nx1...nx2][ny1...ny2]
 *  by a staggered grid finite difference scheme of arbitrary (FDORDER) order accuracy in space
 *  and second order accuracy in time for the elastic VTI problem
 *   
 *  Daniel Koehn
 *  Kiel, 02.02.2017
 *  ----------------------------------------------------------------------*/
        ...
        if((POS[2]==NPROCY-1) && (j>=ny2-FW+1)){
			...
			vxy = vxy / K_y_half[h1] + psi_vxy[h1][i];
				...
				uxy[j][i] = c44h[j][i] * (vyx+vxy) / DT;
			      ...
			      sxy[j][i] += c44h[j][i] * (vyx+vxy);
			      sxx[j][i] += c11[j][i] * vxx + c13[j][i] * vyy;
			      syy[j][i] += c13[j][i] * vxx + c33[j][i] * vyy;
