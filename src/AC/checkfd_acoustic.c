/*-------------------------------------------------------------
 *  Check FD-Grid for stability and grid dispersion for the acoustic case.
...
void checkfd_acoustic(FILE *fp, float ** prho, float ** ppi, float *hc){
	...
	float  c, cmax_p=0.0, cmin_p=1e9, fmax, gamma;
	...
	if (MYID==0){
		...
		fprintf(fp," Minimum and maximum P-wave velocity within subvolumes: \n ");
