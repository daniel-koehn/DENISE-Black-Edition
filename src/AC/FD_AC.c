/*
 * Solve 2D acoustic forward problem by finite-differences
 *
 * Daniel Koehn
 * Kiel, 10/06/2017
 */
...
/* define data structures for AC problem */
struct waveAC;
struct waveAC_PML;
struct matAC;
struct mpiPSV;
...
/* deallocate memory for acoustic forward problem */
...
if (nsrc_loc>0){	
 ...
 if (L) {
		free_matrix(matAC.ptaup,-nd+1,NY+nd,-nd+1,NX+nd);
		free_vector(matAC.peta,1,L);
		free_vector(matAC.etaip,1,L);
		free_vector(matAC.etajm,1,L);
		free_vector(matAC.bip,1,L);
		free_vector(matAC.bjm,1,L);
		free_vector(matAC.cip,1,L);
		free_vector(matAC.cjm,1,L);
		free_matrix(matAC.f,-nd+1,NY+nd,-nd+1,NX+nd);
		free_matrix(matAC.g,-nd+1,NY+nd,-nd+1,NX+nd);
		free_f3tensor(matAC.dip,-nd+1,NY+nd,-nd+1,NX+nd,1,L);
		free_f3tensor(matAC.d,-nd+1,NY+nd,-nd+1,NX+nd,1,L);
		free_f3tensor(matAC.e,-nd+1,NY+nd,-nd+1,NX+nd,1,L);
}
