...
def calc_max_freq(vs,para):
	...
	minvel = np.min(vs[np.nonzero(vs)])
	print("Minimum Vs: ", minvel, " m/s")
...
def check_stability(vs,para):
	...
	maxvel = np.max(vs[np.nonzero(vs)])
	print("Maximum Vs: ", maxvel, " m/s")
...
def do_plot(n, model, cm, an, title, vpmin, vpmax, x, y, font):
    ...
    plt.text(0.1, 1.0,an,fontdict=font,color='white')
    plt.tight_layout()

def plot_model(vs,rho,Q,x,y,cmap,vsmin,vsmax,rhomin,rhomax,Qmin,Qmax,size_x,size_y):
	...
	#rcParams['figure.figsize'] = size_x, size_y

	plt.close('all')
	plt.figure(figsize=(size_x, size_y))
	do_plot(1, vs, cmap, '(a)', r"$\rm{V_s [m/s]}$", vsmin, vsmax, x, y, font)
	do_plot(2, rho, cmap, '(b)', r"$\rm{\rho [kg/m^3]}$", rhomin, rhomax, x, y, font)
	do_plot(3, Q, cmap, '(c)', r"$\rm{Qs [ ]}$", Qmin, Qmax, x, y, font)
...
def plot_acq(vs,xrec,yrec,xsrc,ysrc,x,y,cmap,vsmin,vsmax):
	...
	plt.imshow(vs, cmap, interpolation='none', extent=[np.min(x),np.max(x),np.min(y),np.max(y)])
	...
	cbar.set_label(r"$\rm{V_s [m/s]}$", fontdict=font, labelpad=10)
	plt.tight_layout()
	plt.savefig('model_acq.pdf', bbox_inches='tight', format='pdf')
	plt.show()
...
def write_denise_para(para):
	...
	fp.write("#\n")
	fp.write("#----- Apply 2D Median filter to models ----- #\n")
	fp.write("apply_spatial_filtering_(1=yes)_(MODEL_FILTER) = " + str(para["MODEL_FILTER"]) + "\n")
	fp.write("filter_length_in_gridpoints_(FILT_SIZE) = " + str(para["FILT_SIZE"]) + "\n")
	fp.write("#\n")
	...
	fp.write("MIN_ITER = " + str(para["MIN_ITER"]) + "\n")
	fp.write("#\n")
	fp.write("# ----- Apply 2D Median filter to gradients ----- #\n")
	fp.write("apply_spatial_filtering_(yes=1)_(GRAD_FILTER) = " + str(para["GRAD_FILTER"]) + "\n")
	fp.write("filter_length_in_gridpoints_(FILT_SIZE_GRAD) = " + str(para["FILT_SIZE_GRAD"]) + "\n")
	fp.write("#\n")
